...
const MeasureUnitScreen = ({ navigation }) => {
  ...
  ] = usePagination(useLazyGetMeasureUnitsQuery)
  ...
  const [
    ...
    fixedCacheKey: 'add-measure-unit',
  })
    ...
    if (isCanAdd) {
      navigation.setOptions({
        headerRight: () => (
          <IconButton
            onPress={() => {
              navigation.navigate('MasterMeasureUnitDetailScreen', {
                type: 'add',
              })
            }}
            key="ghost"
  ...
  return (
    <Box flex="1" bgColor="white" paddingX="4">
      <Input
        ref={searchRef}
        placeholder={t('searchMeasureUnit')}
        width="100%"
        borderRadius="4"
        py="3"
        px="1"
...
export default MeasureUnitScreen
